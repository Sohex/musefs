...
//!
//! ID3v2.4.0 structure §5 (<https://id3.org/id3v2.4.0-structure>): "For every
//! new tag that is found, the old tag should be discarded unless the update flag
//! in the extended header (section 3.2) is set." §3.2 defines the flag: "If this
//! flag is set, the present tag is an update of a tag found earlier in the
...
//! Which frames are unique, and by what, is set out frame by frame in the frames
//! document (<https://id3.org/id3v2.4.0-frames>).
//!
//! Two consequences follow. ID3v2.3 and earlier have no update flag, so a run of
//! such tags resolves to the last of them. And "earlier" is file order: §5's
//! search finds appended tags scanning backwards, but §3.2 speaks of a tag
//! "earlier in the present file or stream", and in a stream, the case the flag
//! was designed for, tags arrive in file order.
...
pub fn read_metadata(front: &[u8], tail: &[u8], file_len: u64, bounds: &Mp3Bounds) -> Mp3Metadata {
    ...
    for extent in &bounds.id3v2_tags {
        ...
        if id3v2::is_update(tag) {
            merged.update_with(contents);
        } else {
            merged = contents;
        }
    }
...
impl Mp3Metadata {
    ...
    /// Fold in `later`, a tag flagged as an update. Each of its frames overrides
    /// the corresponding frames merged so far, where "corresponding" follows the
    /// frames document at the grain the store keeps:
    ///
    ...
    fn update_with(&mut self, later: Self) {
        ...
        self.binary_tags
            .retain(|b| !later.binary_tags.iter().any(|l| same_unique_frame(b, l)));
        self.binary_tags.extend(later.binary_tags);
    }
}

/// Does `later`, from an update tag, override `earlier`? Both are binary
/// frames, kept byte-exact; the uniqueness rules quoted are the frames
/// document's.
fn same_unique_frame(earlier: &EmbeddedBinaryTag, later: &EmbeddedBinaryTag) -> bool {
