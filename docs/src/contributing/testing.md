...
and in `musefs-fuse` the mount seams (`FuseConfig`'s forced admission,
directory-handle and stateless-listing caps, and the worker-pool route trace)
that let a mounted test put the daemon in a state it cannot otherwise reach on
demand. Each is switched on by a self-dev-dependency (plus `musefs-core`'s
dev-dependency on `musefs-db`, and `musefs-fuse`'s on `musefs-core`), so no flag
is needed. A new test hook goes behind that feature when an integration test
needs it, and is `#[cfg(test)] pub(crate)` when only the crate's own unit tests
do. A plain `cargo build` does not compile any of it; check such a change with
