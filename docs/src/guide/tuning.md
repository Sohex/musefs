...
| `--workers` | `0` (auto: 2× CPUs) | Worker threads for offloaded FUSE ops (reads, metadata synthesis). The auto default oversizes the pool because the work is I/O-bound. Each worker lazily opens its own read-only SQLite connection, so **steady-state memory scales with this** (see [Memory footprint](#memory-footprint)); lower it on memory-constrained or many-core hosts. A saturated pool slows the mount rather than failing requests: `musefs_pool_over_cap_total` counts the metadata jobs that met the queue cap (see [Admission to the worker pool](../architecture/serving.md#admission-to-the-worker-pool)). |
