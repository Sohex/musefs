...
| `--read-ahead-prefetch` | disabled | Advanced: add background prefetch threads on top of read amplification. Worth enabling on **high-latency network backing** — over NFS at 200 ms round-trip it reads a single stream ~30% faster than amplification alone (6.3 → 8.2 MB/s) and finishes four concurrent streams ~5% sooner. Off by default because that is the only backing where it pays: on local disk and low-latency storage it reads the stream a second time speculatively (about 2x the backing bytes) for no gain. |
| `--keep-cache <true\|false>` | `true` | Keep the kernel page cache across opens. **On by default** — it is the one measured storage win: repeat opens of a file are served from cache instead of re-read over slow storage (~3× faster reopen on HDD/NFS in our benches). A re-tag recorded in the store drops the affected files' cached pages at the mount's next refresh. An in-place rewrite of a *backing* file writes nothing to the store, so a file that is already open can keep serving its cached bytes; the next open fails with `EIO` if the rewrite changed the file's size, mtime, ctime or inode (a same-size rewrite in place on a filesystem with coarse timestamps can change none of them). Disable with `--keep-cache false` (e.g. on a memory-constrained host where the page cache is contended). |
...
| `--trust-backing-mtime` | disabled | Serve `getattr` from the metadata cache without re-statting the backing file. Worth setting only where a `stat` is expensive — NFS, SMB, a spun-down array — since the cache hit otherwise pays one synchronous round trip per track on every traversal after the first. `open` and every read that reaches musefs keep validating, so the flag lets no byte of a changed backing file through musefs; what goes stale is the size/mtime a `stat` reports for a backing file changed without a corresponding store update, until the next `open`. It has no bearing on pages the kernel already cached under `--keep-cache` (see that row). See [freshness](../architecture/tree-scanning.md#freshness-two-version-counters). |
...
| `--workers` | `0` (auto: 2× CPUs) | Worker threads for offloaded FUSE ops (reads, metadata synthesis). The auto default oversizes the pool because the work is I/O-bound. Each worker lazily opens its own read-only SQLite connection, so **steady-state memory scales with this** (see [Memory footprint](#memory-footprint)); lower it on memory-constrained or many-core hosts. A saturated pool slows metadata ops rather than failing them: over the 4096-job admission cap a `lookup`, `getattr` or `open` runs on the thread that submitted it, and `musefs_pool_over_cap_total` counts the jobs that met the cap. Reads are gated separately and are refused with `EAGAIN` once 1024 are in flight (see [Admission to the worker pool](../architecture/serving.md#admission-to-the-worker-pool)). |
