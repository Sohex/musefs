...
| `--trust-backing-mtime` | disabled | Serve `getattr` from the metadata cache without re-statting the backing file. Worth setting only where a `stat` is expensive — NFS, SMB, a spun-down array — since the cache hit otherwise pays one synchronous round trip per track on every traversal after the first. `open` and the read paths keep validating, so no stale bytes are served; what goes stale is the size/mtime a `stat` reports for a backing file changed without a corresponding store update. See [freshness](../architecture/tree-scanning.md#freshness-two-version-counters). |
