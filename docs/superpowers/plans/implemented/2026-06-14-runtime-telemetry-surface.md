...
> **Post-implementation update:** the cargo-feature approach above was abandoned during implementation. `cargo test --workspace` unifies features, so a default-on `musefs/jemalloc → … → musefs-fuse/jemalloc` chain linked the vendored jemalloc C lib into the `musefs-fuse` test binary without it being the global allocator — which **segfaults on FreeBSD** (base libc malloc *is* jemalloc). The shipped design returns to injection, but with a write-once `fn` pointer rather than the spec's `Arc<dyn Fn>`: `musefs-fuse` exposes `set_alloc_probe(fn() -> Option<AllocatorStats>)` (a process-global `OnceLock`) and the binary installs `jemalloc_stats` at startup. `musefs-fuse` carries no `jemalloc` feature and stays allocator-agnostic; `FuseConfig` is still untouched. See `musefs/src/main.rs` and `musefs-fuse/src/lib.rs`.
