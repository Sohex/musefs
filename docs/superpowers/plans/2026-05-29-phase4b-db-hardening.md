...
In `.github/workflows/ci.yml`, in the `check` job, add two steps immediately after the "Property tests (fuzzing feature)" step (after line ~35). The clippy step is required because the existing "Clippy" step (line 31) runs without `--features mutants`, so the gated `Default` code would otherwise never be linted in CI:

```yaml
      - name: DB mutants-feature clippy
        run: cargo clippy -p musefs-db --features mutants --all-targets -- -D warnings
...
> Reframed: `gc_orphan_art` is one `DELETE … WHERE id NOT IN (…)` — no in-process race. **Already covered (do not re-add):** the keep-referenced / remove-orphan / `removed == 1` return value is fully asserted by the existing `gc_orphan_art_removes_unreferenced_rows` (`art.rs:137-140`); and `upsert_art` dedup id-stability by `identical_bytes_dedup_to_one_row`. The genuinely-new gaps are **multi-reference GC** (art shared by two tracks) and **`set_track_art` replace semantics**.
...
Run: `cargo test -p musefs-db --test art shared_art_survives_until_last_reference_gone set_track_art_replaces_links`
Expected: both **PASS**.

- [ ] **Step 3: Hand-apply spot-checks**

- In `musefs-db/src/art.rs::gc_orphan_art`, change `NOT IN` to `IN` → `shared_art_survives_until_last_reference_gone` must FAIL (the still-referenced art would be wrongly removed at the 0-removed assertion). Revert.
...
cargo fmt --all -- --check \
  ...
  && cargo clippy -p musefs-db --features mutants --all-targets -- -D warnings \
