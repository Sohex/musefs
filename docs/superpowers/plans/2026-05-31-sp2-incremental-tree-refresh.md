...
## Task B2: `VirtualTree` introducing-id + mutation primitives

Add the data and helpers needed for in-place mutation, with focused unit tests. "Introducing id" = the lowest track id that introduces a child (a file's own id; a subdir's min over routed tracks).

**Files:**
- Modify: `musefs-core/src/tree.rs`

- [ ] **Step 1: Write failing unit tests for `remove_track` + empty-ancestor prune**

Add to `tree.rs` test module:

```rust
#[test]
fn remove_track_prunes_empty_ancestors() {
    ...
    t.remove_track(10, &mut alloc); // removes x.flac; A and A/B become empty
    assert!(t.inode_of_track(10).is_none());
    assert!(t.lookup(VirtualTree::ROOT, "A").is_none(), "empty A pruned");
    assert!(t.lookup(VirtualTree::ROOT, "C").is_some(), "C untouched");
}

#[test]
fn insert_track_creates_dirs_and_leaf() {
    let mut alloc = InodeAllocator::new();
    let mut t = VirtualTree::build_with(&[(10, "A/x.flac".into())], &mut alloc);
    t.insert_track(20, "A/D/y.flac", &mut alloc);
    let a = t.lookup(VirtualTree::ROOT, "A").unwrap();
    let d = t.lookup(a, "D").unwrap();
    assert!(t.lookup(d, "y.flac").is_some());
    assert_eq!(t.inode_of_track(20), t.lookup(d, "y.flac"));
}
```

- [ ] **Step 2: Run to verify failure**

Run: `cargo test -p musefs-core --lib remove_track_prunes -- --nocapture`
Expected: FAIL — `no method named remove_track`.

- [ ] **Step 3: Implement `remove_track` and `insert_track`**

Add to `impl VirtualTree`:

```rust
/// Remove the file node for `track_id` (if present) and prune any ancestor
/// directories left empty. Does NOT re-disambiguate siblings — the caller batches
/// that. The retired path's inode is never recycled (allocator is monotonic).
pub fn remove_track(&mut self, track_id: i64, _alloc: &mut InodeAllocator) {
    let Some(ino) = self.track_to_inode.remove(&track_id) else { return };
    ...
    self.prune_empty_dirs_upward(parent);
}

/// Walk upward from `dir`, removing any directory whose children map is empty.
fn prune_empty_dirs_upward(&mut self, mut dir: u64) {
    while dir != Self::ROOT {
        let empty = self.children.get(&dir).is_none_or(|c| c.is_empty());
        if !empty { break; }
...
/// Insert a file node at `path` (already-disambiguated component-wise is NOT
/// assumed; this uses the same ensure_dir/disambiguate as build). The caller is
/// responsible for any re-disambiguation needed for byte-identical equivalence.
pub fn insert_track(&mut self, track_id: i64, path: &str, alloc: &mut InodeAllocator) {
    self.insert_file(track_id, path, alloc);
}
```

> `is_none_or` requires a recent Rust; if the toolchain rejects it, use `self.children.get(&dir).map_or(true, |c| c.is_empty())`.

- [ ] **Step 4: Run to verify pass**

Run: `cargo test -p musefs-core --lib "remove_track_prunes|insert_track_creates" -- --nocapture`
Expected: PASS.
...
git commit -m "feat(core): VirtualTree remove_track/insert_track + empty-dir prune (SP2 B2)"
```

---

## Task B3: Canonical-order re-disambiguation (`redisambiguate_dir`)

The equivalence-critical primitive: rebuild one directory's child name-bindings by clearing them and re-interning children in **ascending introducing-id** order. This reclaims freed base names and reproduces `build_with` exactly.

**Files:**
- Modify: `musefs-core/src/tree.rs`

- [ ] **Step 1: Write failing tests (reclamation + dir-vs-file + sibling-subdir swap)**

```rust
#[test]
fn redisambiguate_reclaims_freed_base_name() {
    let mut alloc = InodeAllocator::new();
    // Two tracks collide in dir D: id 10 -> song.flac, id 20 -> song (2).flac.
    let mut t = VirtualTree::build_with(
    ...
    // Remove the holder of the base name (id 10), then re-disambiguate D.
    t.remove_track(10, &mut alloc);
    t.redisambiguate_dir(d, &mut alloc);

    // id 20 reclaims song.flac (matches a full rebuild).
...
Then an equivalence cross-check helper test for the sibling-subdir reorder (compare against `build_with` directly):

```rust
// Build the same final entry set two ways and compare path->inode maps.
...
#[test]
fn sibling_subdir_reorder_matches_full_rebuild() {
    // Two albums render the same dir name "Q". id1,id5 -> Q ; id3 -> Q (2).
    let initial = vec![
        (1, "Q/x.flac".to_string()),
        (5, "Q/y.flac".to_string()),
        (3, "Qb/z.flac".to_string()),  // see note: force same rendered dir name "Q"
    ];
    // (In a real test the two albums must render the SAME dir name to collide;
    //  construct entries whose dir components are literally equal. Use:
    //  (1,"Q/x.flac"),(5,"Q/y.flac") in dir Q, and a THIRD track that also wants
    //  dir "Q" but is a different album -> via build it becomes "Q (2)". Model
    //  that by giving the third the path "Q/z.flac" too, so all three collide as
    //  FILES, OR construct a genuine dir collision. Keep this test focused on the
    //  property oracle in Task B5 if a literal dir-name collision is awkward here.)
    let _ = initial;
}
```

> The cleanest place to prove the sibling-subdir swap is the **property oracle in Task B5**, which compares against `build_with` over random edits. Keep the unit tests here to the deterministic reclamation + dir-vs-file cases; rely on B5 for the cascade. Replace the sketchy `sibling_subdir_reorder_matches_full_rebuild` above with a concrete dir-vs-file test:

```rust
#[test]
fn redisambiguate_dir_vs_file_collision_matches_build() {
    let mut alloc = InodeAllocator::new();
    // id 1 -> file "A/Foo" ; id 2 -> file under dir "A/Foo/bar.flac" (dir Foo).
    // Build order (ascending id) decides who claims "Foo".
    let entries = vec![(1, "A/Foo".to_string()), (2, "A/Foo/bar.flac".to_string())];
    let reference = VirtualTree::build(&entries);
    let mut alloc2 = InodeAllocator::new();
    let mut t = VirtualTree::build_with(&entries, &mut alloc2);
    let a = t.lookup(VirtualTree::ROOT, "A").unwrap();
    t.redisambiguate_dir(a, &mut alloc2);
    let ra = reference.lookup(VirtualTree::ROOT, "A").unwrap();
    assert_eq!(
...
Run: `cargo test -p musefs-core --lib redisambiguate -- --nocapture`
Expected: FAIL — `no method named redisambiguate_dir`.

- [ ] **Step 3: Implement introducing-id + `redisambiguate_dir`**

```rust
/// The introducing id of a child inode under a directory: a file's own track id;
/// a subdirectory's minimum track id over all file descendants. Pure function of
/// the current subtree. Used to order re-disambiguation identically to a full
/// build (which processes tracks in ascending id). See SP2 Component 3.
fn introducing_id(&self, ino: u64) -> i64 {
    match self.nodes.get(&ino).map(|n| &n.kind) {
        Some(NodeKind::File { track_id }) => *track_id,
        _ => {
            // min over descendant files
            let mut min = i64::MAX;
            let mut stack = vec![ino];
            while let Some(n) = stack.pop() {
                if let Some(NodeKind::File { track_id }) = self.nodes.get(&n).map(|x| &x.kind) {
                    min = min.min(*track_id);
                }
                if let Some(kids) = self.children.get(&n) {
                    for (_, &c) in kids.iter() { stack.push(c); }
                }
            }
            min
        }
...
/// Re-derive every child name-binding in `dir` to match a fresh build: clear the
/// current bindings and re-intern each child (by current introducing id ascending)
/// through the same disambiguation rule. Renames retire old paths and intern new
/// ones via the persistent allocator. Recurses into renamed subdirs so descendant
/// inodes (keyed on full path) stay consistent. See SP2 Component 3.
pub fn redisambiguate_dir(&mut self, dir: u64, alloc: &mut InodeAllocator) {
    let dir_path = self.path_of(dir);
    // Snapshot current children as (introducing_id, old_inode, is_dir, old_base_name).
    let mut kids: Vec<(i64, u64, bool, String)> = match self.children.get(&dir) {
        Some(c) => c.iter().map(|(name, &ino)| {
            (self.introducing_id(ino), ino, self.is_dir(ino), strip_disambig(name))
        }).collect(),
        None => return,
    };
    kids.sort_by_key(|(iid, ino, _, _)| (*iid, *ino));

    // Clear and rebuild the children map for `dir`, re-assigning names.
    let mut new_children: OrdMap<String, u64> = OrdMap::new();
    let mut renamed: Vec<(u64, String, String)> = Vec::new(); // (ino, old_full, new_full)
    for (_iid, ino, is_dir, base) in kids {
        let unique = disambiguate_in(&new_children, &base);
        let new_full = join_path(&dir_path, &unique);
        let old_name = self.nodes.get(&ino).map(|n| n.name.clone()).unwrap_or_default();
        let old_full = join_path(&dir_path, &old_name);
        // Re-point the node's name + inode to the canonical path's inode.
        let new_ino = alloc.intern(&new_full);
        new_children.insert(unique.clone(), new_ino);
        if new_ino != ino || unique != old_name {
            renamed.push((ino, old_full, new_full));
        }
        // Move node record to new_ino with updated name (and re-key track/children).
        self.rekey_node(ino, new_ino, dir, unique, is_dir);
    }
    self.children.insert(dir, new_children);
    // Recurse into subdirs (their full path changed iff renamed) to fix descendants.
    for (_ino, _old, _new) in &renamed {
        // descendant fix handled by rekey_node walking children; see helper.
    }
}
```

> **Implementation reality check:** the rename/rekey of subtree inodes (paths are keys) is the fiddly part. Two viable strategies — choose ONE and implement it fully (the property oracle in B5 is the proof):
> 1. **Subtree rebuild on rename:** if a directory child's disambiguated name changes, drop that child's whole subtree and re-`insert_track` every descendant track at its new path (via `alloc.intern`). Simpler to reason about; cost is O(subtree) which is bounded by the colliding group. RECOMMENDED.
> 2. **In-place rekey:** walk the subtree re-interning each node's full path. More code, same result.
>
> For strategy 1, `redisambiguate_dir` becomes: snapshot the dir's tracks (descendant `(track_id, relative_path_below_dir)`), clear the dir's subtree from the maps, then re-`insert_file` each track in ascending track-id order — which makes `disambiguate`/`ensure_dir` reproduce the canonical naming exactly. This reuses the existing, already-tested `insert_file` path and needs no `rekey_node`. Prefer this; delete the `rekey_node`/`renamed` scaffolding above.

Rewrite `redisambiguate_dir` per strategy 1:

```rust
pub fn redisambiguate_dir(&mut self, dir: u64, alloc: &mut InodeAllocator) {
    // Collect (track_id, full_path) for every file at or below `dir`.
    let mut tracks: Vec<(i64, String)> = Vec::new();
    let mut stack = vec![dir];
    while let Some(n) = stack.pop() {
        match self.nodes.get(&n).map(|x| x.kind.clone()) {
            Some(NodeKind::File { track_id }) => tracks.push((track_id, self.path_of(n))),
            _ => {
                if let Some(kids) = self.children.get(&n) {
                    for (_, &c) in kids.iter() { stack.push(c); }
                }
    ...
    // Remove them all (prunes now-empty dirs under `dir`).
    let ids: Vec<i64> = tracks.iter().map(|(id, _)| *id).collect();
    for id in &ids { self.remove_track(*id, alloc); }
    // Re-insert in ascending track-id order so disambiguate reproduces build_with.
    tracks.sort_by_key(|(id, _)| *id);
    for (id, path) in tracks { self.insert_file(id, &path, alloc); }
}
```

Add the small helpers (`path_of`, and make `disambiguate` usable over an `OrdMap` — it already reads `self.children[&dir]`, so no change needed; remove the unused `strip_disambig`/`disambiguate_in`/`rekey_node` sketches):
...
> Note: with strategy 1, `redisambiguate_dir(dir)` re-inserts using the SAME path strings, so `disambiguate` re-derives names against the freshly-cleared dir — which is exactly the canonical-order rebuild. The introducing-id ordering is achieved by re-inserting in ascending track-id order (a file's introducing id IS its track id; a subdir's is the min of its files, which ascending-track-id insertion naturally respects because the first track to reach a subdir is its min id). So `introducing_id` is not strictly needed for strategy 1 — keep it only if a later task needs it; otherwise delete it to avoid dead code (clippy will flag it).
...
cargo fmt && cargo clippy -p musefs-core --all-targets
git add musefs-core/src/tree.rs
git commit -m "feat(core): canonical-order redisambiguate_dir (clear+reinsert) (SP2 B3)"
```

---

## Task B4: Wire in-place mutation into `rebuild_incremental` with fallback

Replace the `build_with` call in `rebuild_incremental` with: clone the current tree (O(1), im), apply remove/insert/redisambiguate per the `ChangeSet`, and publish. On any caught inconsistency, fall back to `build_full`.

**Files:**
- Modify: `musefs-core/src/facade.rs`, `musefs-core/src/tree.rs`

- [ ] **Step 1: Add a tree method that applies a ChangeSet**

In `tree.rs`, add a method that takes the new snapshot's paths and the change sets and mutates self. Keep it dependency-light by passing plain data (not `ChangeSet`, to avoid `tree.rs` depending on `refresh_diff`):

```rust
/// Apply an incremental change set in place: remove `removed` + the old side of
/// path-moves, insert `added` + the new side of path-moves, then re-disambiguate
/// each affected directory's subtree (canonical order). `new_paths` maps every
/// CURRENT track id to its rendered path. `removed`/`changed`/`added` are id lists.
/// Returns Err(()) if an expected node is missing (caller falls back to full build).
...
pub fn apply_changes(
    ...
    let mut affected_dirs: HashSet<u64> = HashSet::new();

    // Helper: parent dir inode for a path, creating ancestors; returns dir inode.
    // For removals we use the existing node's parent.

    // 1+2: remove `removed` and the OLD side of any changed track whose path moved.
    for &id in removed {
        if let Some(ino) = self.inode_of_track(id) {
            if let Some(p) = self.node(ino).map(|n| n.parent) { affected_dirs.insert(p); }
        }
        self.remove_track(id, alloc);
    }
    for &id in changed {
        let new_path = match new_paths.get(&id) { Some(p) => p, None => return Err(()) };
        if let Some(ino) = self.inode_of_track(id) {
            let old_path = self.path_of(ino);
            if &old_path != new_path {
                if let Some(p) = self.node(ino).map(|n| n.parent) { affected_dirs.insert(p); }
                self.remove_track(id, alloc);
                self.insert_file(id, new_path, alloc);
                if let Some(ni) = self.inode_of_track(id) {
                    if let Some(p) = self.node(ni).map(|n| n.parent) { affected_dirs.insert(p); }
                }
            }
            // else: path unchanged -> nothing structural.
        } else {
            // Was expected to exist; treat as add.
            self.insert_file(id, new_path, alloc);
            if let Some(ni) = self.inode_of_track(id) {
                if let Some(p) = self.node(ni).map(|n| n.parent) { affected_dirs.insert(p); }
            }
    ...
    // 4: re-disambiguate affected directories (canonical order via clear+reinsert).
    for dir in affected_dirs {
        if self.node(dir).is_some() {
            self.redisambiguate_dir(dir, alloc);
        }
    }
    Ok(())
}
...
> `insert_file` is currently private; `apply_changes` is on `impl VirtualTree` so it can call it. Keep `insert_file` private.

- [ ] **Step 2: Use it in `rebuild_incremental`, with fallback**

In `facade.rs::rebuild_incremental`, replace the entries-assembly + `build_with` tail with:

```rust
    // Build new_paths from the snapshot.
    let new_paths: std::collections::HashMap<i64, String> =
        new_snapshot.iter().map(|(&id, s)| (id, s.path.clone())).collect();

    let mut alloc = self.inodes.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let mut tree = (*self.tree.load_full()).clone(); // O(1) im clone
    match tree.apply_changes(&new_paths, &change.changed, &change.added, &change.removed, &mut alloc) {
        Ok(()) => {
            if cfg!(debug_assertions) {
                // Debug-only equivalence assert against build_with on a clone of the live allocator.
        ...
        Err(()) => {
            // Fallback: full rebuild for this refresh (logged).
            eprintln!("musefs: incremental tree mutation failed; falling back to full rebuild");
            let mut entries: Vec<(i64, String)> =
                new_paths.iter().map(|(&id, p)| (id, p.clone())).collect();
            entries.sort_by_key(|(id, _)| *id);
            let full = VirtualTree::build_with(&entries, &mut alloc);
            self.tree.store(Arc::new(full));
        }
    }
    drop(alloc);
    Ok((new_snapshot, change))
...
> `InodeAllocator` must be `Clone` for the debug assert. Add `#[derive(Clone)]` to it in `tree.rs` (it currently derives only `Debug`).

- [ ] **Step 3: Add `VirtualTree::equiv` for the debug assert**

In `tree.rs`:

```rust
/// Structural equality for the equivalence oracle: same path->inode file mapping
/// and same directory structure. Compares the (track_id -> inode) and the full
/// path set. See SP2 Testing item 1.
pub fn equiv(&self, other: &VirtualTree) -> bool {
    self.track_to_inode == other.track_to_inode
        && self.nodes.len() == other.nodes.len()
        && self.nodes.iter().all(|(ino, n)| other.nodes.get(ino) == Some(n))
}
```

> `Node` and `im::HashMap` both impl `PartialEq` (Node derives it; `im::HashMap<K,V>: PartialEq` when `V: PartialEq`). If `im::HashMap` equality is not derivable directly, compare via `self.nodes.iter().all(...)` as written (already does).

- [ ] **Step 4: Run the Stage A equivalence test against the now-incremental tree**

Run: `cargo test -p musefs-core --test incremental_refresh -- --nocapture`
Expected: PASS — incremental mutation now produces the same tree as a full rebuild.

Run: `cargo test -p musefs-core -- --nocapture`
Expected: PASS (all unit + integration, including the debug_assert active in test builds).
...
git commit -m "feat(core): in-place tree mutation in rebuild_incremental + fallback (SP2 B4)"
```

---

## Task B5: Property-test oracle (random edit sequences)

The headline gate: many random edit sequences, incremental tree ≡ `build_with(entries, allocator.clone())` after every step, including collision churn and the sibling-subdir reorder.

**Files:**
- Modify: `musefs-core/tests/incremental_refresh.rs` (or a new `musefs-core/tests/proptest_incremental.rs`)

- [ ] **Step 1: Write the proptest**

Add (proptest is already a dev-dependency in this workspace — confirm via `grep -n proptest musefs-core/Cargo.toml`; if missing, add `proptest` under `[dev-dependencies]`):

```rust
use proptest::prelude::*;

// Strategy: a sequence of edits over a fixed small id space, where edits set
// template fields that deliberately collide (to force disambiguation churn).
#[derive(Clone, Debug)]
enum Edit { Retag(usize, String, String), Delete(usize) }
...
proptest! {
    ...
    fn incremental_equivalent_to_full_under_random_edits(
        edits in proptest::collection::vec(
            prop_oneof![
                (0usize..6, "[A-C]", "[x-y]").prop_map(|(i,a,t)| Edit::Retag(i, a, t)),
                (0usize..6).prop_map(Edit::Delete),
            ], 0..20)
    ) {
        let dir = tempfile::tempdir().unwrap();
        let corpus = dir.path().join("corpus");
        common::corpus::write_min_corpus(&corpus, 6);
        let db_path = dir.path().join("musefs.db");
        let db = Db::open(&db_path).unwrap();
        scan_directory(&db, &corpus).unwrap();
        let fs = Musefs::open(Db::open(&db_path).unwrap(), config()).unwrap();
        ...
        for e in edits {
            match e {
                Edit::Retag(i, album, title) => {
                    if let Some(&id) = ids.get(i) {
                        // Deliberately collide: same album+title -> same path.
                ...
                Edit::Delete(i) => { if let Some(&id) = ids.get(i) { let _ = writer.delete_track(id); } }
            }
            fs.poll_refresh().unwrap();
            // Reference: a fresh full Musefs over the current DB state.
            let reference = Musefs::open(Db::open(&db_path).unwrap(), config()).unwrap();
            prop_assert_eq!(
                tree_fingerprint(&fs).keys().collect::<Vec<_>>(),
                tree_fingerprint(&reference).keys().collect::<Vec<_>>()
...
> Note: comparing path *sets* (the `keys()`) proves structural equivalence. Inode-identity equivalence is enforced by the in-process `debug_assert` in B4 (which runs during these test builds and uses the live allocator clone) — so the proptest path-set check + the active debug_assert together cover Testing item 1's "identical inode assignments" claim. If you want an explicit inode cross-check here, two independently-built `Musefs` instances will NOT share allocator history, so their inode *numbers* legitimately differ — do not assert raw inode equality across the two instances; rely on the in-process debug_assert for that.

- [ ] **Step 2: Run it (expect to surface any divergence)**

Run: `cargo test -p musefs-core incremental_equivalent_to_full -- --nocapture`
Expected: PASS. If it fails, proptest prints a minimal failing edit sequence — that is the exact divergence case to fix in `apply_changes`/`redisambiguate_dir` (most likely the sibling-subdir reorder; ensure `affected_dirs` includes the parent of any renamed subdir and that `redisambiguate_dir` recurses via clear+reinsert).

- [ ] **Step 3: Commit**

```bash
cargo fmt && cargo clippy -p musefs-core --all-targets
git add musefs-core/tests/incremental_refresh.rs musefs-core/Cargo.toml
git commit -m "test(core): property oracle - incremental tree == full build_with (SP2 B5)"
```

---

## Task B6: Fallback test

**Files:**
- Modify: `musefs-core/src/tree.rs` (test-only injection), `musefs-core/tests/incremental_refresh.rs`

- [ ] **Step 1: Add a way to force an inconsistency**

Simplest deterministic injection: a `#[doc(hidden)] pub fn force_apply_failure_for_test(&self, on: bool)` on `Musefs` that makes `rebuild_incremental` take the `Err(())` branch once. Implement via an `AtomicBool` field `force_apply_fail` checked at the top of the `apply_changes` call site: if set, skip `apply_changes` and go straight to the fallback path, then clear the flag.
...
fn apply_failure_falls_back_to_full_rebuild() {
    ...
    // Tree still equals a full rebuild despite taking the fallback path.
    let reference = Musefs::open(Db::open(&db_path).unwrap(), config()).unwrap();
    assert_eq!(
        tree_fingerprint(&fs).keys().collect::<Vec<_>>(),
        tree_fingerprint(&reference).keys().collect::<Vec<_>>(),
...
- [ ] **Step 3: Run to verify (fail → implement injection → pass)**

Run: `cargo test -p musefs-core apply_failure_falls_back -- --nocapture`
Expected: FAIL (missing accessor) → add it → PASS.

- [ ] **Step 4: Commit**

```bash
cargo fmt && cargo clippy -p musefs-core --all-targets
git add musefs-core/src/facade.rs musefs-core/tests/incremental_refresh.rs
git commit -m "test(core): Stage B fallback to full build_with on injected failure (SP2 B6)"
```

---

## Task B7: Stage B bench + regression gate

**Files:**
- Modify: `musefs-core/tests/bench_refresh.rs`, `BENCHMARKS.md`, spec README results log

- [ ] **Step 1: Re-run the library-size sweep**

Run: `cargo test -p musefs-core --test bench_refresh bench_refresh_one_across_library_sizes -- --ignored --nocapture`
Expected: `refresh-1@N` wall time now **flat across N** (strict O(changed)) — contrast with the Stage A baseline recorded in A7.

- [ ] **Step 2: Run the full regression gate**

Run: `cargo test`
Expected: PASS (all crates; excludes `#[ignore]` FUSE e2e).

Run the FUSE e2e (needs `/dev/fuse`): `cargo test -p musefs-fuse -- --ignored`
Expected: PASS (byte-identical audio holds; SP2 does not touch the read path).

Run the read-path regression check: `cargo bench -p musefs-core --bench read_throughput` and confirm the `ci` `sequential_read` median is within 10% of the prior run (SP2 must not regress reads).

- [ ] **Step 3: Record Stage B numbers**

Append Stage B rows to `BENCHMARKS.md` (Stage A vs Stage B refresh-1-by-size, refresh-N) and a results-log line to the spec README. Update the README status row: `| SP2 | Implemented | SP2-incremental-tree-refresh.md | 2026-05-31-sp2-incremental-tree-refresh.md | ... |`.
...
git add musefs-core/tests/bench_refresh.rs BENCHMARKS.md docs/superpowers/specs/2026-05-30-optimization-pass/README.md
git commit -m "bench(core): Stage B refresh flat across library size; SP2 done (SP2 B7)"
```
