...
| P2-13 | Zero-byte art boundary (#16) | Add `musefs-format/tests/synthesize_art.rs` (extend existing art tests) | **✅ Remediated (Phase 3 + ogg follow-up).** Zero-byte art is skipped at synthesis for every format — FLAC (3a), MP3/WAV (3b), MP4 (3c), and Ogg (`ogg::synthesize_layout` filters `data_len > 0` across Opus/Vorbis/OggFLAC, test `synthesize_opus_skips_zero_byte_art`). |
